...
// 在 action node 中添加幫 action 計數器 + 1 的 statement
...
// 在 `if/else`, `with` node 中添加幫 branch 計數器 + 1 的 statement
...
// 在 `range` node 中添加幫 branch 計數器 + 1 的 statement
...
func (t *TreeStrategy) insertProbes(template []byte) []byte {
	...
	// 在每個 template 中添加 global 計數器 dict:
	// - action: 對應 action node
	// - branch: 對應 action node
	// - loop: 對應 action node
...
// Create a new transformer for each template
